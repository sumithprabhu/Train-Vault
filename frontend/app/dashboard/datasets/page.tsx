...
export default function DatasetsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await apiGet<{ datasets: DatasetItem[] }>("/dataset")
  ...
  const filtered = datasets.filter(
    (d) =>
      d.cid.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} className="p-6 text-muted-foreground">Loading…</td></tr>
                ) : filtered.length === 0 ? (
                  <tr><td colSpan={6} className="p-6 text-muted-foreground">No datasets</td></tr>
                ) : (
                  paginated.map((d) => (
                    <tr
                      key={d.cid}
          ...
          {selected && (
            <div className="space-y-3 text-xs">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <a href={`https://calibration.filfox.info`} target="_blank" rel="noopener noreferrer">
                  <Button size="sm" variant="outline"><ExternalLink size={12} className="mr-1" /> Explorer</Button>
                </a>
              </div>
            </div>
          )}
